...
@section('pageTitle',trans('Selected Do'))
                                    ...
                                    <div class="row">
                                        <div class="col-lg-4 col-md-6 col-sm-12">
                                            <div class="form-group mb-3">
                                                ...
                                                @if($user)
                                                    <select class="form-select supplier_id" name="supplier_id" required onchange="getDistProduct();">
                                                ...
                                                @else
                                                    <select class="form-select supplier_id" name="supplier_id" onchange="getBalance(); getDistProduct();" required>
                                        ...
                                        <div class="col-lg-3 col-md-6 col-sm-12">
                                            <div class="form-group mb-3">
                                                <label class="py-2" for="product">{{__('Group')}}<span class="text-danger">*</span></label>
                                                <select class="select2 form-select" id="group_id" onchange="groupWiseProductShow(this);">
                                                    <option value="">Select</option>
                                                    {{-- @forelse ($group as $pg)
                                                    <option class="" value="{{ $pg->id }}">{{ $pg->name }}</option>
                                                    @empty
                                                    @endforelse --}}
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group mb-3">
                                                ...
                                                @else
                                                    <select class=" form-select" id="product_id">
                                                        ...
                                                        <option class=" {{$pro->distributor_id}}" data-dp='{{ $pro->dp_price }}' data-unit='{{ $pro->unit_style?->unit?->qty }}' data-name='{{ $pro->product_name }}' data-ratio='{{ $pro->free_ratio }}' data-free='{{ $pro->free }}' value="{{ $pro->id }}">{{ $pro->product_name }}</option>
                                                        @empty
                                                        @endforelse --}}
                                                    </select>
                                                @endif
...
<script>
    ...
    function groupWiseProductShow(e){
        let group_id= $(e).val();
        $('#product_id').find('.selecet_p_hide').hide();
        $('#product_id').find('.selecet_p_hide'+group_id).show();
    }
    let old_supplier_id=0;
    let opt="";
    let optGroup="";
    function getDistProduct() {
        let supplier_id=$('.supplier_id').val();
        
        if(old_supplier_id!=supplier_id){
            $.ajax({
                url: "{{ route(currentUser().'.get_selected_dist_product') }}",
                type: "GET",
                dataType: "json",
                ...
                success: function(datas) {
                    ...
                    if(datas.data.length > 0){
                        for(d of datas.data){
                            opt+=`<option class="selecet_p_hide selecet_p_hide${d.group_id}" data-dp='${d.dp_price}' data-name='${d.product_name }' data-unit='${d.unit_qty }' data-ratio='${d.free_ratio}' data-free='${d.free}' value="${d.id}">${d.product_name}</option>`;
                        }

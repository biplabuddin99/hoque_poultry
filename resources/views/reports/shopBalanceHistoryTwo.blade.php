                            ...
                            <table class="tbl_border" style="width:100%">
                                <tbody>
                                    ...
                                    <tr class="tbl_border">
                                        ...
                                        <td class="tbl_border" style="text-align: center; padding: 5px;">
                                            {{money_format($outAmount - $inAmount)}}
                                        </td>
                                    </tr>
                                    @empty
                                    @endforelse --}}
                                    {{-- @foreach ($dueCollection as $d)
                                        <tr class="tbl_border">
                                            ...
                                            <td class="tbl_border" style="text-align: center; padding: 5px;">
                                                {{money_format($outAmount-$inCollectAmount)}}
                                            </td>
                                        </tr>
                                    @endforeach --}}
                                    {{--dd($data)--}}
                                    @foreach($data as $d)
                                        <tr class="tbl_border">
                                            ...
                                            <td class="tbl_border" style="text-align: center; padding: 5px;">
                                                @if ($d->status == '0' || $d->check_type == 1)
                                                    {{ number_format($d->balance_amount - $d->check_collect_amount, 2) }}
                                                    @php $outAmount += $d->balance_amount - $d->check_collect_amount; @endphp
                                                @endif
                                            </td>
                                            <td class="tbl_border" style="text-align: center; padding: 5px;">
                                                @if ($d->status == '1' && $d->balance_amount > 0.00 && $d->check_collect_amount == 0.00)
                                                    @php $a = $d->balance_amount ;@endphp
                                                    {{ number_format($d->balance_amount, 2) }}
                                                    @else
                                                     {{ number_format($d->check_collect_amount, 2) }}
                                                       @php $a = $d->check_collect_amount ;@endphp
                                                    @php $inAmount += $a; @endphp
                                                @endif
                                            </td>
                                            <td class="tbl_border" style="text-align: center; padding: 5px;">
                                                {{ number_format($outAmount - $inAmount, 2) }}
                                            </td>
                                        </tr>
                                    @endforeach

...
@section('pageTitle',trans('Selected Sales Update'))
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <form method="post" action="{{route(currentUser().'.sales.update',encryptor('encrypt',$sales->id))}}" onsubmit="return confirm('are you sure to Save?')">
                            @csrf
                            @method('PATCH')
                            <div class="row p-2 mt-4">
                                ...
                                <div class="col-lg-3 col-md-6 col-sm-12 mt-2">
                                    ...
                                    <input type="text" class="form-control" value="{{$sales->distributor?->name}}">
                                </div>
                                <div class="col-lg-3 col-md-6 col-sm-12 mt-2 d-none">
                                    <label for="cat">{{__('Distributor')}}<span class="text-danger">*</span></label>
                                    @if($user)
                                ...
                                <div class="col-lg-3 mt-2" id="area">
                                    ...
                                    <select class="form-select area_id" name="area_id" id="area_name" onchange="shop_area_wise();" required>
                                        ...
                                        @foreach ($area_name as $a)
                                            <option value="{{$a->id}}" {{$sales->area_id == $a->id? 'selected' : '' }}>{{$a->name}}</option>
                                        @endforeach
                                ...
                                <div class="col-lg-3 mt-2 " id="shopNameContainer" style="display: none;">
                                    ...
                                    <select class="form-select" name="shop_id" id="shop_deselect">
                                        ...
                                        @forelse ($shops as $p)
                                            <option value="{{$p->id}}" {{$sales->shop_id == $p->id? 'selected' : ''}}> {{$p->shop_name}} ({{$p->area_name}})</option>
                                        @empty
                                            <option value="">No Data Found</option>
                                        @endforelse
                                    </select>
                                ...
                                <div class="col-lg-5 mt-2" id="dsrNameContainer" style="display: none;">
                                    <div class="row">
                                        <div class="col-8">
                                            ...
                                            <select class="form-select" name="dsr_id" id="dsr_deselect">
                                                ...
                                                @forelse ($userDsr as $p)
                                                    <option value="{{$p->id}}" {{$sales->dsr_id == $p->id? 'selected' : '' }}>{{$p->name}}</option>
                                                @empty
                                                    <option value="">No Data Found</option>
                                                @endforelse
                                            </select>
                                        ...
                                        <div class="col-4">
                                            ...
                                            <input type="text" class="form-control receive_dsr_amount" name="receive_amount" value="{{$sales->receive_amount}}">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-3 mt-2">
                                    ...
                                    <table class="table table-bordered mb-0 table-striped">
                                        <thead>
                                            <tr class="text-center">
                                                ...
                                                <th scope="col" width="10%">{{__('Group')}}</th>
                                        ...
                                        <tbody id="sales_repeat">
                                            @forelse ($salesDetails as $sd)
                                                <tr>
                                                    <td>{{++$sl}}</td>
                                                    <td>
                                                        <select class="select2 group_product" id="group_id{{$sd->id}}" name="group_id[]" onchange="groupWiseProductShow(this);">
                                                            <option value="">Select Group</option>
                                                            @foreach ($group as $gp)
                                                                <option value="{{$gp->id}}" {{$sd->group_id == $gp->id? 'selected' : ''}}>{{$gp->name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <select class="form-control form-select product_id" id="product_id" onchange="productData(this);" name="product_id[]">
                                                            ...
                                                            @foreach ($product as $p)
                                                                <option class="selecet_p_hide selecet_p_hide{{$p->group_id}}" data-tp='{{$p->tp_price}}' data-tp_free='{{$p->tp_free}}' value="{{$p->id}}" {{$sd->product_id == $p->id? 'selected' : ''}}>{{$p->product_name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td><input class="form-control ctn" onkeyup="productData(this);" onblur="productData(this);" onchange="productData(this);" type="text" name="ctn[]" value="{{$sd->ctn}}"></td>
                                                    <td><input class="form-control pcs" onkeyup="productData(this);" onblur="productData(this);" onchange="productData(this);" type="text" name="pcs[]"value="{{$sd->pcs}}"></td>
                                                    <td>
                                                        <select class="form-select select_tp_tpfree" name="select_tp_tpfree[]" onchange="productData(this);">
                                                            <option value="2" {{$sd->select_tp_tpfree==2? 'selected' : ''}}>TP Free</option>
                                                            <option value="1" {{$sd->select_tp_tpfree==1? 'selected' : ''}}>TP</option>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <input class="form-control ctn_price" type="hidden" name="ctn_price[]" value="{{$sd->ctn_price}}" >
                                                        <input class="form-control totalquantity_pcs" type="text" value="{{$sd->totalquantity_pcs}}">
                                                    </td>
                                                    <td><input class="form-control per_pcs_price" type="text" name="per_pcs_price[]" value="{{$sd->pcs_price}}" onkeyup="manulPcsPrice(this);" ></td>
                                                    <td>
                                                        <input class="form-control subtotal_price" type="text" name="subtotal_price[]" value="{{$sd->subtotal_price}}">
                                                        <input class="form-control totalquantity_pcs" type="hidden" name="totalquantity_pcs[]" value="{{$sd->totalquantity_pcs}}">
                                                    </td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <select class="form-control form-select product_id" id="product_id" onchange="productData(this);" name="product_id[]">
                                                            <option value="">Select Product</option>
                                                            @foreach ($product as $p)
                                                                <option class="selecet_p_hide selecet_p_hide{{$p->group_id}}" data-tp='{{$p->tp_price}}' data-tp_free='{{$p->tp_free}}' value="{{$p->id}}">{{$p->product_name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th class="totalctn text-center" id="totalCtn"></th>
                                                <th class="totalPcs text-center" id="totalPcs"></th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                    <div class="row mb-1">
                                        ...
                                        <div class="col-lg-2 mt-2 text-end" style="margin-left: 3rem!important;">
                                            <label for="" class="form-group"><h5 class="total" id="grandTotal">{{$sales->total}}</h5></label>
                                            <input type="hidden" name="total" class="total_p" id="grandTotalP" value="{{$sales->total}}">
                                        </div>
                                        <div class="col-lg-3"></div>
                                    </div>
                                    <div class="row">
                                        <div class="col-6" id="oldDueContainer" style="display: none;">
                                            <table class="table">
                                                ...
                                                <tbody id="shop-due">
                                                    @forelse ($tempShpBalance as $tempSb)
                                                        <tr>
                                                            <td>
                                                                <select name="old_due_shop_id[]" class="form-control form-select due_shop" id="deselect_shop_due" onchange="getShopDue(this);">
                                                                    <option value="">Select Shop</option>
                                                                    @forelse ($shops as $p)
                                                                        <option value="{{$p->id}}" {{$tempSb->old_due_shop_id == $p->id? 'selected' : ''}}> {{$p->shop_name}} ({{$p->area_name}})</option>
                                                                    @empty
                                                                        <option value="">No Data Found</option>
                                                                    @endforelse
                                                                </select>
                                                            </td>
                                                            <td><input type="text" class="form-control shop_due_amount text-end" name="due_amount[]" value="{{$tempSb->due_amount}}"></td>
                                                            <td class="text-center">
                                                                <span onClick='removeShop(this);' class="delete-row text-danger"><i class="bi bi-trash-fill"></i></span>
                                                                <span onClick='addShop();' class="add-row text-primary"><i class="bi bi-plus-square-fill"></i></span>
                                                            </td>
                                                        </tr>
                                                    @empty
                                                        <tr>
                                                            <td>
                                                                <select name="old_due_shop_id[]" class="form-control form-select due_shop" id="deselect_shop_due" onchange="getShopDue(this);">
                                                                    <option value="">Select Shop</option>
                                                                    @forelse ($shops as $p)
                                                                        <option value="{{$p->id}}"> {{$p->shop_name}} ({{$p->area_name}})</option>
                                                                    @empty
                                                                        <option value="">No Data Found</option>
                                                                    @endforelse
                                                                </select>
